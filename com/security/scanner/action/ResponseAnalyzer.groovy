 ...
 * If the unique "CrossSiteScriptingAcademia" + a constant value is found then the attack is deemed
 * successful
 *
 * @author: Adrian Rapan
 *
...
package com.security.scanner.action

import com.security.scanner.model.AttackVector
...
class ResponseAnalyzer {
  ...
  */
   ...
   * If the unique "CrossSiteScriptingAcademia" string is found on the HTML response then the attack is successful
  ...
  def validateResponse(AttackVector attackVector) {
    ...
    if (attribute.isEmpty()) {
      ...
      if (reader.'**'.findAll {
        ...
        it.name().toLowerCase() == element && it.text().contains('CrossSiteScriptingAcademia' + toLookFor)
      }.size() >= 1) {
      ...
      if (reader.'**'.findAll {
        it.name().toLowerCase() == element && it.attributes().find {it.value.contains('CrossSiteScriptingAcademia' + toLookFor)}
      }.size() >= 1) {
        println 'Attack was successful with vector ' + code
        Statistics.toLog += 'Attack was successful with vector ' + code + '\n'
        Statistics.successful++
