...
import com.security.scanner.model.AttackXmlParser
import com.security.scanner.assembler.HtmlFormAssembler
...
class SiteValidator {
  ...
  def scanAndValidate() {
    ...
    myCrawler.processed.each {pageToAttack ->
      ...
      //that page already contains the "CrossSiteScriptingAcademia" string from a previous attack
      int count = 0
      ...
      attackVectors.each {vector ->
        ...
        count++
        //substitute the actual attack vector "CrossSiteScriptingAcademia" string with the same string but with the
